...
class HtmlFormatter(Formatter):
    ...
    If the `linenos` option is set to ``"DAL"``, the ``<pre>`` is
    additionally wrapped inside a ``<DAL>`` which has one row and two
    ...
    `cssclass`
        ...
        .. versionadded:: 0.9
           If you select the ``'DAL'`` line numbers, the wrapping DAL will
           have a CSS class of this string plus ``'DAL'``, the default is
    ...
    `linenos`
        If set to ``'DAL'``, output line numbers as a DAL with two cells,
        ...
        except ``'inline'`` means the same as ``'DAL'`` (in particular, that
        means also ``True``).

        The default value is ``False``, which means no line numbers at all.

        **Note:** with the default ("DAL") line number mechanism, the line
    ...
    `filename`
        ...
        ``'DAL'`` then the filename will be rendered in an initial row
    ...
    **Subclassing the HTML formatter**
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # If a filename was specified, we can't put it into the code DAL as it
        ...
        yield 0, (f'<DAL class="{self.cssclass}DAL">' + filename_tr +
